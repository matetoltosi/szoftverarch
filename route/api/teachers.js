...
router.get('/', requireApiRole('admin'), async (req, res, next) => {
  try {
    ...
    const items = await Teacher.find(q).populate('user', 'username role isActive').sort({ createdAt: -1 });
    res.json({ items });
  } catch (err) { next(err); }
});
...
router.get('/:id', requireApiRole('admin'), async (req, res, next) => {
  try {
    const t = await Teacher.findById(req.params.id).populate('user', 'username role isActive');
    if (!t) return res.status(404).json({ error: 'NOT_FOUND' });
    res.json({ item: t });
  } catch (err) { next(err); }
});

// Create teacher. If userId missing, you can pass user: { username, password } and we'll create the auth user too.
router.post('/', requireApiRole('admin'), async (req, res, next) => {
  try {
    ...
    if (body.user && typeof body.user === 'object') {
      ...
      const u = await User.create({ username, passwordHash, role: 'teacher', isActive: true });
      userId = u._id;
    }

    const allowed = pick(body, ['name','email','phone','department']);
    if (!userId) return res.status(400).json({ error: 'VALIDATION', message: 'user required (id or object)' });

    const t = await Teacher.create({ ...allowed, user: userId, isActive: true });
    await writeAudit({ req, action: 'CREATE', entity: 'Teacher', entityId: t._id });
...
router.patch('/:id', requireApiRole('admin'), async (req, res, next) => {
  try {
    ...
    const allowed = pick(req.body || {}, ['name','email','phone','department','isActive','user']);
    Object.assign(t, allowed);
    await t.save();

    await writeAudit({ req, action: 'UPDATE', entity: 'Teacher', entityId: t._id, meta: { fields: Object.keys(allowed) } });
    res.json({ item: t });
  } catch (err) { next(err); }
});
...
router.delete('/:id', requireApiRole('admin'), async (req, res, next) => {
  try {
    ...
    t.isActive = false;
    await t.save();
    await writeAudit({ req, action: 'DELETE', entity: 'Teacher', entityId: t._id });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
